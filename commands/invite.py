...
def register_invite_command(app: Client):
    ...
    async def invite(client: Client, message: Message):
        ...
        if len(args) > 0:     
            await message.reply("Ouch, you wrote something wrong. Use /help invite to learn how to use this function.")
            return
        if await vote.vote(message, client, f"Should an invite link be sent to {message.from_user.first_name}?", 60*60*12):
            # Adding
            try:
                link_json = await client.create_chat_invite_link(message.chat.id)
                link = dict(json.loads(str(link_json)))["invite_link"]
                await message.reply(f"An invitation has been sent to {message.from_user.first_name}")
                # Send a message to the person who started the vote
                await client.send_message(
                    chat_id=message.from_user.id,
                    text=f"Join here --> {link}")
